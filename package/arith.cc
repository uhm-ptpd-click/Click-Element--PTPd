...
void 
fromInternalTime(TimeInternal * internal, PTP_Timestamp * external)
{

	/*
	 * fromInternalTime is only used to convert time given by the system
	 * to a PTP_Timestamp As a consequence, no negative value can normally
	 * be found in (internal)
	 * 
	 * Note that offsets are also represented with TimeInternal structure,
	 * and can be negative, but offset are never convert into PTP_Timestamp
	 * so there is no problem here.
	 */

	if ((internal->seconds & ~INT_MAX) || 
	    (internal->nanoseconds & ~INT_MAX)) {
		DBG("Negative value canno't be converted into PTP_Timestamp \n");
		return;
	} else {
...
void 
toInternalTime(TimeInternal * internal, PTP_Timestamp * external)
{

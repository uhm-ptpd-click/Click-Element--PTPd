...
/** \brief Display a PTP_Timestamp Structure*/
void 
PTP_Timestamp_display(PTP_Timestamp * PTP_Timestamp)
{
	uInteger48_display(&PTP_Timestamp->secondsField);
	DBGV("nanoseconds %u \n", PTP_Timestamp->nanosecondsField);
}
...
{
	DBGV("Message Sync : \n");
	PTP_Timestamp_display(&sync->originPTP_Timestamp);
	DBGV("\n");
}
...
msgAnnounce_display(MsgAnnounce * announce)
{
	...
	DBGV("\n");
	DBGV("originPTP_Timestamp : \n");
	DBGV("secondField  : \n");
	PTP_Timestamp_display(&announce->originPTP_Timestamp);
...
{
	PTP_Timestamp_display(&follow->preciseOriginPTP_Timestamp);
}
...
{
	PTP_Timestamp_display(&req->originPTP_Timestamp);
}
...
{
	PTP_Timestamp_display(&resp->receivePTP_Timestamp);
...
{
	PTP_Timestamp_display(&preq->originPTP_Timestamp);
}
...
msgPDelayResp_display(MsgPDelayResp * presp)
{
	...
	PTP_Timestamp_display(&presp->requestReceiptPTP_Timestamp);
...
msgPDelayRespFollowUp_display(MsgPDelayRespFollowUp * prespfollow)
{
	...
	PTP_Timestamp_display(&prespfollow->responseOriginPTP_Timestamp);
